...
` `CORR(SYSDATE - DATA\_ADMISSAO, SALARIO) AS CORRELACAO
...
`                 `THEN INSTR('1' ||SYS\_CONNECT\_BY\_PATH(t3.ixqnty, '\*'),'\*', **1**, **8**)  + **1** 
...
`            `CASE WHEN INSTR('1' ||SYS\_CONNECT\_BY\_PATH(t3.ixqnty, '\*'),'\*', **1**, **16**) > **0** 
